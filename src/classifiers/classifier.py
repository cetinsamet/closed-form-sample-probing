...
class CLASSIFIER:
    ...
    def compute_per_class_acc_gzsl(self, test_label, predicted_label, target_classes):
        ...
        for i in target_classes:
            idx = (test_label == i)
            acc_per_class += torch.sum(test_label[idx]==predicted_label[idx]) / (torch.sum(idx) + 1e-6)
    ...
    def compute_per_class_acc(self, test_label, predicted_label, nclass):
        ...
        for i in range(nclass):
            idx = (test_label == i)
            acc_per_class[i] = torch.sum(test_label[idx]==predicted_label[idx]) / (torch.sum(idx) + 1e-6)
